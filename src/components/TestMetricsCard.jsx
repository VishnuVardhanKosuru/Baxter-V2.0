...
export default function TestMetricsCard({ pipelineState }) {
  const isCompleted = pipelineState === 'completed';
  const isRunning = pipelineState === 'running';

  const seleniumCount = isCompleted ? 14 : isRunning ? '...' : 0;
  const cucumberCount = isCompleted ? 8 : isRunning ? '...' : 0;
  const totalCount = isCompleted ? 22 : isRunning ? '...' : 0;
  ...
  return (
    <section className="baxter-card" style={{ padding: '1.05rem 1.35rem' }}>
      ...
      <div style={{ marginTop: '0.85rem', padding: '0.6rem 0.9rem', backgroundColor: '#F0F7FF', borderRadius: 8, border: '1px solid #BFDBFE', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: '0.825rem', fontWeight: 600, color: '#002670' }}>Total Generated Tests</span>
        <span style={{ fontSize: '0.95rem', fontWeight: 800, color: '#0033A0' }}>{totalCount} Suites</span>
      </div>
    </section>
  );
}
